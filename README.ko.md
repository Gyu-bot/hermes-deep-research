...
`deep`과 `exhaustive`는 여러 조사 갈래를 한꺼번에 띄웁니다. 그런데 Hermes의 서브에이전트 동시 실행 수 기본값이 그보다 낮게 잡혀 있는 경우가 많습니다. 처음 제대로 돌리기 전에 이 설정을 확인하고, 한 웨이브가 띄울 갈래 수(대략 조사 축 하나당 하나)보다 낮다면 올려 두세요.

값이 낮다고 해서 고장 나지는 않습니다. 메인 에이전트가 같은 웨이브 안에서 갈래를 여러 배치로 나눠 순차 실행하고, 런타임 제약에 맞춰 조사 기준을 낮추지도 않습니다. 다만 그만큼 오래 걸리고, 모드 예산이 실제 경과 시간 기준이라 순차 실행으로 늘어진 `deep`·`exhaustive` 조사는 충분히 수렴하기 전에 한도에 걸려 `partial`로 끝날 수 있습니다.

이 스킬은 해당 설정을 직접 지정하거나 바꾸지 않습니다. 런타임 제약은 작업을 띄울 때의 상한으로만 취급하므로, 조정은 사용자의 판단에 맡깁니다.
...
- 서브에이전트 동시 실행 수를 확인해 현재 값을 사용자에게 알리고, 조사 축 하나당 하나에 못 미치면 웨이브가 순차 실행되어 긴 조사가 `partial`로 끝날 수 있다는 점을 설명합니다. 바꾸기 전에 반드시 사용자에게 확인하고, 말없이 올리지 않습니다.
...
init/validate/status 실행 검사를 하세요. 서브에이전트 동시 실행 수가 병렬
조사에 충분한지 확인해서 알려 주되, 바꾸기 전에 먼저 물어보세요. 자격 증명을
