...
어려운 질문을 일반 검색 스킬에 던지면 페이지 하나가 돌아옵니다. 한 번에 끝내는 리서치 스킬에 던지면 한 차례 검색하고 한 번 요약한 결과가 돌아옵니다. 주장이 실제로 엇갈리는 주제에서는 이런 방식이 예측 가능한 방식으로 무너집니다.
...
| 끝날 때까지 아무것도 저장하지 않아 세션이 끊기면 작업이 사라짐 | 단계마다 노트·출처·구체적인 다음 할 일을 파일로 저장해, 대화 기록이 없는 새 세션도 이어받을 수 있음 |
...
모든 과정은 Hermes 표준 도구(웹·브라우저, 파일, 터미널, 보조 에이전트)만으로 돌아갑니다. 상주 프로세스도 외부 서비스도 쓰지 않습니다. 문서나 PDF는 별개의 결과물이며, 명시적으로 요청했을 때만 만듭니다.
