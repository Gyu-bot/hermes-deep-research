...
설치 후 `SKILL.md`를 읽어 보세요. 새 실행은 모두 `$HERMES_HOME/research/hermes-deep-research/` 아래에 만들며, 환경 변수가 없으면 `~/.hermes`를 사용합니다. 실행에서 생기는 파일은 해당 실행 디렉터리 안에만 두고, 설치된 스킬 디렉터리를 조사 작업공간으로 쓰지 않습니다.

Hermes cron은 무인 조사를 할 때만 필요합니다.

### 먼저 서브에이전트 동시 실행 수를 확인하세요

조사 갈래는 `delegate_task`로 띄우고, Hermes는 동시에 도는 자식 에이전트 수를 `delegation.max_concurrent_children`으로 제한합니다. **기본값은 3**입니다. `deep` 웨이브는 보통 그보다 많은 갈래를 계획합니다.

이 제한은 알아서 조정해 주는 값이 아니라 강제되는 값입니다. Hermes 공식 문서에 따르면 제한보다 큰 배치는 "조용히 잘려 나가는 대신 도구 오류를 반환"합니다. 대기열에 넣어 주지도, 잘라 주지도 않습니다. 메인 에이전트는 현재 제한을 읽어 그에 맞게 배치를 나누도록 지시받으므로 정상 동작에서는 오류 대신 순차 실행이 됩니다. 다만 3으로 두면 갈래 여섯 개를 계획한 웨이브가 두 번의 순차 배치로 돌아가 그만큼 오래 걸리고, 모드 예산이 실제 경과 시간 기준이라 `deep`·`exhaustive` 조사가 충분히 수렴하기 전에 예산에 걸려 `partial`로 끝날 수 있습니다.

그래서 처음 제대로 돌리기 전에 한 웨이브가 띄울 갈래 수만큼 올려 두는 편이 좋습니다. `~/.hermes/config.yaml`에서:

```yaml
delegation:
  max_concurrent_children: 8   # 기본값 3, 최소 1, 상한 없음
```
...
동시에 도는 자식이 늘면 속도만이 아니라 토큰 소비도 같이 늘어납니다. 감당할 만한 수치로 정하세요. 설정 이름과 기본값은 [Hermes 위임 문서](https://hermes-agent.nousresearch.com/docs/user-guide/features/delegation)에서 확인할 수 있습니다. 이 스킬은 해당 값을 직접 지정하거나 바꾸지 않고, 런타임 제약을 작업을 띄울 때의 상한으로만 취급합니다.

### 선택 외부 스킬

완성된 조사 결과를 읽기 좋은 보고서로 만들어 주는 외부 스킬이 두 개 있습니다. 하나는 문장을, 하나는 문서를 담당합니다. 이 저장소에 포함되어 있지 않고 자동으로 설치되지도 않습니다. 각각 별도 프로젝트이므로 해당 프로젝트의 최신 안내를 따라 직접 설치해야 합니다. 최종 보고서를 결과물로 받을 생각이라면 함께 설치하기를 권합니다.

| 스킬 | 무엇을 위한 스킬인가 | 저장소 |
| --- | --- | --- |
| **Bookforge** | 문서 디자인 — 승인된 보고서를 잘 조판된 PDF로 만들어, 지면에서 읽기 좋게 합니다. PDF를 만드는 유일한 경로입니다 | [gongnyang/bookforge](https://github.com/gongnyang/bookforge) |
| **Humanize Korean** | 문장 다듬기 — 한국어 문장에서 AI 티와 번역투를 걷어내 사람이 쓴 글처럼 읽히게 합니다 | [epoko77-ai/im-not-ai](https://github.com/epoko77-ai/im-not-ai) |

조사 결과를 다른 작업의 참고 자료로만 쓸 때는 둘 다 쓰지 않습니다. 사람이 앉아서 최종 보고서를 읽는 경우를 위한 스킬입니다. 각 저장소에서 직접 설치하고 호환성을 확인한 뒤 사용하세요. 해당 스킬의 스크립트나 지침을 이 스킬 안으로 복사해 오지는 않습니다. 조사 자체는 둘 없이도 완결됩니다. 필요한 연동이 없으면 검증된 Markdown 결과물을 전달하면서 그 사실을 밝히며, PDF나 다듬은 원고를 만들었다고 말하지 않습니다. 각각을 어떤 조건에서 쓰는지는 [선택 기능](#-선택-기능)에 있습니다.
...
- `delegation.max_concurrent_children`(기본값 3)을 확인해 현재 값을 사용자에게 알리고, 계획한 갈래 수보다 낮으면 웨이브가 순차 실행되어 긴 조사가 `partial`로 끝날 수 있다는 점을 설명합니다. 바꾸기 전에 반드시 사용자에게 확인하고, 말없이 올리지 않습니다.
- 선택 외부 스킬은 각자의 저장소에서 따로 설치하며, 요청한 결과물에 필요할 때만 설치하고, 먼저 해당 프로젝트의 최신 안내와 호환성을 확인합니다.
- 관련 없는 도구를 설치하거나 자격 증명, 공급자, 다른 설정을 바꾸지 않습니다.
- 아래 개발 섹션의 테스트와 임시 `create` → `validate` → `status` 점검을 실행합니다.
...
create/validate/status 실행 검사를 하세요. delegation.max_concurrent_children
설정이 병렬 조사에 충분한지 확인해서 알려 주되, 바꾸기 전에 먼저 물어보세요. 자격 증명을
바꾸거나 관련 없는 도구를 설치하지 마세요. 요청한 결과물에 필요할 때만 선택
기능을 설치하고, 먼저 해당 프로젝트의 최신 안내와 호환성을 확인하세요.
```
...
```text
<run-dir>/
├── state.json                  # 질문, 모드, 조사 축, 이력, 한계, 다음 할 일
├── sources.json                # 보존하는 출처 목록
├── report.md                   # 보존하는 최종 Markdown 보고서
├── notes/                      # 메인 에이전트가 통합한 보존 노트
├── lanes/<wave>/<lane>/
│   └── result.md               # 보존하는 보조 에이전트 결과
└── tmp/                        # 삭제 가능한 자료만 저장
    ├── workspace/
    ├── raw-pages/
    ├── raw-data/
...
가져온 원문, 다운로드, 추출물, 일회성 스크립트, lane 작업 파일은 `tmp/` 아래에 둡니다. 보고서·출처 목록·통합 노트·lane 결과가 이 삭제 가능 영역을 가리키면 검증에서 거부합니다. 실행이 종료 상태가 된 뒤 `research_state.py cleanup <run-dir>`을 실행하면 삭제 없이 임시 파일 수와 용량만 보여 줍니다. 실제 삭제는 별도로 승인한 `--apply` 실행에서만 일어납니다.
...
조사가 만들어 내는 것은 정확한 보고서입니다. 이 두 스킬은 그 보고서를 읽기 좋은 보고서로 만들어 주며, 담당하는 층이 다릅니다. **Humanize Korean**은 문장이 어떻게 읽히는지를, **Bookforge**는 완성된 문서가 어떻게 보이는지를 손봅니다. 둘 다 조사 결과 자체는 건드리지 않습니다.

두 스킬 모두 최종 보고서가 결과물일 때, 즉 사람이 앉아서 읽을 문서를 만들 때만 씁니다. 조사 결과를 다른 작업에 넣을 내부 메모로 쓰는 경우에는 Markdown과 출처 목록이 그대로 다음 작업으로 넘어가고 두 스킬은 실행되지 않습니다. 또한 둘 다 조사가 끝난 뒤에만 동작합니다. `completed`/`partial` 판단이 이미 내려진 다음에 실행되므로, 문장을 다듬거나 조판하는 일이 근거를 만들거나 조사 상태를 바꿀 수 없습니다.

둘 다 각자의 저장소에서 관리되는 외부 스킬이며, [따로 설치](#선택-외부-스킬)해야 하고 자동으로 딸려 오지 않습니다. 조사 자체는 둘 없이도 완결되며, 이 스킬의 의존성은 아닙니다. 쓰기 전에 해당 프로젝트의 최신 안내를 확인하세요.

### Bookforge — 요청한 PDF의 문서 디자인

[Bookforge](https://github.com/gongnyang/bookforge)는 마지막 한 구간을 맡습니다. 승인된 Markdown을 받아 구성과 타이포그래피, 표지를 갖춘 PDF로 조판해, 텍스트 파일을 그대로 뽑은 듯한 문서가 아니라 지면에서 읽기 좋은 문서로 만듭니다. 조판을 위해 제목 구조나 장 경계를 정리할 수는 있지만, 다시 조사하거나 분량을 늘리거나 보고서의 주장을 고쳐 쓰지는 않습니다.

그 앞에는 관문이 두 개 있습니다. 먼저 문서화 준비 관문입니다. 승인된 보고서를 `report.pre-document.md`로 보존하고, 내용을 더하지 않는 편집만으로 `report.document-candidate.md`를 만든 뒤, 메인 에이전트가 `report.document-readiness-gate.md`에 `PASS`/`FAIL` 판단을 직접 기록합니다. 읽기 좋은지는 자동화가 판단하지 않습니다. 그다음 `scripts/document_gate.py pass`가 승인된 파일을 `report.document-ready.md`로 복사하고 SHA-256으로 묶습니다. Bookforge에 넘기기 직전에 `document_gate.py verify`가 성공해야 하며, 실패하면 렌더링을 진행하지 않습니다.
...
### Humanize Korean — 한국어 보고서의 문장 다듬기

[Humanize Korean](https://github.com/epoko77-ai/im-not-ai)은 조판이 아니라 문장을 다룹니다. 내용은 멀쩡한데 읽기 피곤하게 만드는 AI 티 나는 문장, 번역투, 군더더기, 기계적인 리듬을 걷어내면서 의미, 사실, 숫자, 날짜, 고유명사, 인용, 전문 용어, 문서 구조, 링크, 불확실성 표현, 문체는 그대로 지킵니다. 원본이 아니라 복사본에서 실행합니다.

편집 결과는 검증되지 않은 후보로 취급합니다. `report.pre-polish.md`와 비교해 사실, 의미, 확신 수준, 범위, 조건, 충돌하는 내용, 한계, 숫자, 날짜, 이름, 인용, 링크, 구조가 바뀌었다면 되돌리거나 수정합니다. 검증할 수 없으면 편집 전 보고서를 그대로 전달합니다. 문장이 매끄러워지는 대가로 근거 검증을 느슨하게 하지는 않습니다.
...
│   └── LICENSE.md
├── scripts/
│   ├── research_state.py               # 생성 / 초기화 / 조회 / 검증 / 임시 파일 정리
...
`research_state.py`는 고유하고 격리된 실행을 만들고, 보존 경로와 삭제 가능 경로를 구분해 검증하며, 기록된 `tmp/`만 안전하게 조회하거나 비웁니다. 정리는 기본적으로 dry-run이고, 진행 중이거나 legacy 형식인 실행은 거부합니다. 실제 삭제는 검증한 device·inode·type 스냅샷에 결속되므로 도중에 바뀐 경로도 거부합니다. 그 밖에도 상태·모드·계획 상한·현재 웨이브·20% 이상 종합 예산·이유가 붙은 예산 조정·조사 축 형식·`completed`/`partial` 보고서의 비어 있지 않음을 검사합니다. `document_gate.py`는 승인된 문서 후보의 SHA-256 결속을 기록하고 검증합니다. 두 스크립트 모두 조사 품질을 판단하지 않습니다.
