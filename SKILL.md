...
3. Before any search, delegation, script creation, download, extraction, browser action that writes a file, or document generation, resolve the installed skill directory from `skill_view` and create the run with the helper. The canonical base is `$HERMES_HOME/research/hermes-deep-research`, with `HERMES_HOME` falling back to `~/.hermes`. Use a short human-readable slug; the helper adds a UTC timestamp, creates a unique directory, and prints its absolute path:

```bash
RUN_DIR="$(python3 "/absolute/skill/dir/scripts/research_state.py" create \
  "research-topic" \
...
4. Treat the printed absolute `RUN_DIR` as the hard boundary for the whole research unit. Never reuse another run directory. A new run contains:
...
5. Every `write_file` target, terminal-created file or script, browser download, extraction output, and document artifact must use an absolute path inside `RUN_DIR`. Use `RUN_DIR/tmp/workspace` as the terminal workdir. Put one-off scripts, fetched HTML or PDF, search payloads, browser downloads, raw extracts, and transient conversion files only under the matching `tmp/` directory. Do not write research files directly in the home directory, starting cwd, system temporary directory, or installed skill directory. If a tool unavoidably uses tool-managed ephemeral storage, copy every needed artifact into `RUN_DIR` immediately and never present the external transient path as durable run state.
6. Keep `tmp_path` as the relative value `tmp` in `state.json`. Checkpoint state before dispatching work, and keep `next_actions` concrete enough that a fresh parent can continue without conversation history.
...
Plan flat coverage lanes rather than assuming one child per axis. A lane is one independent leaf assignment. The parent may split an axis into lanes by language, source surface, or adversarial perspective and dispatch them in parallel. Do not use nested delegation by default: the parent remains the sole orchestrator and the only writer of parent state, sources, notes, and report files.
...
1. Use `delegate_task` in live interactive sessions for independent, bounded coverage lanes. Before dispatch, assign each lane a temporary workdir at `RUN_DIR/tmp/lanes/<wave>/<lane-id>/` and a durable result at `RUN_DIR/lanes/<wave>/<lane-id>/result.md`. Make every prompt self-contained with the full research question and intended use, one lane, mode, wave purpose, languages and source surfaces, freshness needs, known evidence and duplicate exclusions, required countersearch, diagnostic query and original-fetch ceilings, the stop and coverage definition, both absolute paths and their confinement rules, the output language, and the [research-note template](templates/research-note.md). Ask for readable Markdown, not strict JSON.
   A lane may write temporary material only inside its assigned temporary directory and must preserve its final result at the assigned `result.md`, including a clear no-findings or limitation result when applicable. It must not write elsewhere in `RUN_DIR` or mutate parent `state.json`, `sources.json`, `report.md`, or `notes/`.
...
7. The parent reads each lane result, copies or integrates accepted findings into `notes/`, and adds useful sources to `sources.json` with topic, note, use, and limitation fields. The ledger is the source-to-report map; sentence-level citations are optional.
...
3. For a Korean reader report, use [Humanize Korean](https://github.com/epoko77-ai/im-not-ai) only as an optional editorial integration when that polish matches the user's intent and a compatible installation is available. Run it with genre `report` on a copy under `RUN_DIR/tmp/workspace`, never directly over the canonical draft. If the tool forces tool-managed ephemeral storage, copy the candidate back into `RUN_DIR` before review and do not retain the transient path as run state. Its purpose is to remove AI-sounding Korean, translationese, padding, and mechanical rhythm while preserving meaning, facts, numbers, dates, proper nouns, quotations, technical terms, Markdown structure, links, uncertainty, and register. Do not install it merely to complete deep research.
...
```bash
python3 "/absolute/skill/dir/scripts/research_state.py" validate "$RUN_DIR"
```

## Document only on explicit request

After the normal research synthesis, any applicable editorial pass, status decision, and validation are complete, run [references/report-documentation.md](references/report-documentation.md) only if the user explicitly requested a report document, final document, or PDF. Documentation has exactly two stages: the document-readiness gate, then rendering with the optional external [Bookforge](https://github.com/gongnyang/bookforge) integration. Inspect its current upstream instructions and compatibility before installing or invoking it. Keep Bookforge work under `RUN_DIR/tmp/`; copy the final PDF to durable `RUN_DIR/report.pdf`. The parent records its qualitative Markdown verdict with `scripts/document_gate.py`; the resulting JSON is only the byte-bound machine record. Immediately before handoff, `document_gate.py verify "$RUN_DIR"` must succeed or Bookforge scaffolding and rendering are forbidden. Never treat `reader_report`, `report.md`, a ready file by itself, or a deep-research request by itself as authorization to render. Deep Research remains complete without Bookforge; if it is unavailable, deliver the validated Markdown artifacts and do not claim a PDF was produced.

## Audit containment and retain safely

Before marking a run `completed` or `partial` and again before handoff, audit every durable file produced by the parent, lanes, browser, terminal, scripts, extraction, editorial tools, and document tools. Confirm that each absolute path is inside `RUN_DIR`, every lane has its durable `lanes/<wave>/<lane-id>/result.md`, and parent integration is in `notes/` and `sources.json`. Copy any needed tool-managed ephemeral output into `RUN_DIR`; do not treat the external transient path as durable state.

If an accidental research output is found outside `RUN_DIR`, report it and move or copy it only with the user's direction. Never delete temporary files or old runs automatically. After research is terminal, `python3 "/absolute/skill/dir/scripts/research_state.py" cleanup "$RUN_DIR"` is a dry run that reports file count and bytes. Deletion is a separate action: obtain explicit user approval immediately before adding `--apply`. Cleanup refuses `researching` or `synthesizing` runs and may clear only the recorded `tmp` tree; it never touches state, sources, reports, notes, or lane results.
